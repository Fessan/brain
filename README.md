...
- **Часть 2: Получите ключи** ⏱ 15 мин
  ...
  - [2.4 Singularity API Token](#24-singularity-api-token)
...
| «Перенеси все просроченные задачи на понедельник» | Найдёт в Singularity и перенесёт |
| «Поставь задачу: завтра в 9 утра — зарядка» | Создаст в Singularity |
| «Обнови мои недельные цели» | Откроет файл целей и поможет обновить |
| «Какие задачи на этой неделе не сделаны?» | Покажет список из Singularity |
...
**Про Codex — не бойтесь названия.** Это универсальный AI-агент, который отлично помогает в жизни. У него чёткие инструкции как действовать с вашими записями — создавать задачи, сохранять мысли, следить за целями. Claude можно включить через кнопку в боте.
...
3. **Codex (по умолчанию)** анализирует записи
4. Создаёт задачи в **Singularity**, сохраняет мысли в **Obsidian**
...
| **Singularity** | Хранение задач в одном месте (API требует Pro/Elite) | зависит от тарифа |
...
| `singularity.md` | Проекты в Singularity, приоритеты |
...
| Singularity API Token | me.singularity-app.com → API Access | UUID формат |
...
### 2.4 Singularity API Token

> Важно: доступ к API требует подписки Pro/Elite.

1. Откройте личный кабинет [me.singularity-app.com](https://me.singularity-app.com)
2. Перейдите в раздел **API Access** (или **REST tokens**)
3. Нажмите **Create Token**
4. Выберите права (минимум: tasks и projects)
5. Скопируйте токен и сохраните — он показывается один раз
...
| Singularity API Token | UUID формат |
...
**Вы установили Codex CLI и авторизовались — отлично!**
...
Скрипт сам:
- Скачает ваш проект
- Спросит у вас все токены (Telegram, Deepgram, Singularity)
...
| 🤖 Claude | Переключить активную модель (Codex/Claude) |
...
Эта система использует **MCP (Model Context Protocol)** — открытый стандарт для подключения AI к внешним сервисам. По умолчанию настроен Singularity, но вы можете подключить любой сервис, для которого есть MCP-сервер:

| Сервис | MCP-сервер | Что можно делать |
|--------|------------|------------------|
| **Singularity** | MCP bundle из личного кабинета | Задачи, проекты, привычки *(уже настроен)* |
...
**Проверка MCP Singularity:**
```bash
node .mcp/singularity/mcp.js --help
```
...
SINGULARITY_ACCESS_TOKEN=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
VAULT_PATH=./vault
